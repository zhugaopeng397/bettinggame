...
describe("CasinoP3", async () => {
  it("Not allow you to propose a zero wei bet", async () => {
    f = await ethers.getContractFactory("CasinoP3")
    c = await f.deploy()
  ...
  it("Not allow you to accept a bet that doesn't exist", async () => {
    ...
    try {
      tx = await c.acceptBet(hashA, hashB, {value: 1e10})
      rcpt = await tx.wait()
      expect("this").to.equal("fail")
    } catch (err) {
        expect(interpretErr(err)).to
  ...
  it("Allow you to propose and accept bets", async () => {
    ...
    tx = await c.proposeBet(hashA, {value: 1e10})
    rcpt = await tx.wait()
  ...
  it("Not allow you to accept with the wrong amount", async () => {
    ...
    expect(rcpt.events[0].event).to.equal("BetProposed")
    try {
      tx = await c.acceptBet(hashA, hashB, {value: 1e11})
      rcpt = await tx.wait()   
      expect("this").to.equal("fail")      
    } catch (err) {
        expect(interpretErr(err)).to
  ...
  it("Work all the way through (B wins)", async () => {
    ...
    tx = await cB.revealB(hashA, valB)
    rcpt = await tx.wait()
  ...
  it("Work all the way through (A wins)", async () => {
    ...
    tx = await cA.proposeBet(hashA, {value: 0.1})
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetProposed")

    tx = await cB.acceptBet(hashA, hashBlose, {value: 0.1})
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetAccepted")      
